define([
  ...
  return declare([BaseWidget], {
    ...
    postCreate: function() {
      ...
      topic.subscribe(
        "addLayer",
        lang.hitch(this, this.onTopicHandler_addLayer)
      );
    },

    onOpen: function() {
      //隐藏按钮和面板
      query(".cad-selected-box")
        .style("opacity", 0)
        .style("display", "none");
      query(".selected-cad")
        .style("opacity", 0)
    ...
    _addSublayerCheckBoxClickEvent: function() {
      query(".tab-pane-checkbox a").on("click", function() {
        ...
        sublayerNames =
          "layers=" + sublayerNames.substr(0, sublayerNames.length - 1);
    ...
    _showSublayerInfo: function(layerLabel, sublayerInfos, layerUrl) {
      ...
      query(".checkbox-height").addContent(
        "<div class='tab-pane active' id='" +
          ...
          "<a class='active' data-layerLabel='" +
          layerLabel +
          "'>" +
    ...
    onTopicHandler_addLayer: function(params) {
      ...
      if (
        layerType !== undefined &&
        layerType.toLowerCase() === "ChengDiDynamic".toLowerCase()
      ) {
        //获取服务信息
        this._getSublayerInfo(layerUrl).then(
