...
define([
  ...
  return declare([BaseWidget], {
    ...
    postCreate: function () {
      ...
      topic.subscribe("setBusLineFlow", lang.hitch(this, this.onTopicHandler_setBusLineFlow));
      topic.subscribe("setMetroLineFlow", lang.hitch(this, this.onTopicHandler_setMetroLineFlow));
    },

    _getBusLineGeometry: function (lineName) {
      var def = new Deferred();

      var layerUrl = this.config.busLineUrl.replace(/{gisServer}/i, this.appConfig.gisServer);
      ...
      queryTask.execute(query, lang.hitch(this, function (queryResults) {
        //公交线路图层是按照站点打断的, 要把查询结果合并
        var lineGeometries = array.map(queryResults.features, function (feature) {
          return feature.geometry;
        });
        geometryEngineAsync.union(lineGeometries).then(function (unionedGeometry) {
    ...
    _getMetroLineGeometry: function (lineName) {
      ...
      var layerUrl = this.config.metroLineUrl.replace(/{gisServer}/i, this.appConfig.gisServer);
    ...
    onTopicHandler_setBusLineFlow: function (params) {
      ...
      array.forEach(flows, function (flowData) {
        ...
        if (lineName !== null) {
          this._getBusLineGeometry(lineName).then(lang.hitch(this, function (geometry) {
            var busLineGraphic = new Graphic(geometry);
            busLineGraphic.attributes = {flow: flowData.flow, lineName: lineName};
            busLineGraphic.infoTemplate = new InfoTemplate("${lineName}", "<b>客运量: </b>${flow}人");
    ...
    onTopicHandler_setMetroLineFlow: function (params) {
      ...
      array.forEach(flows, function (flowData) {
        var lineName = flowData.lineName;
        if (lineName !== null) {
          this._getMetroLineGeometry(lineName).then(lang.hitch(this, function (geometry) {
            var metroLineGraphic = new Graphic(geometry);
            metroLineGraphic.attributes = {flow: flowData.flow, lineName: lineName};
            metroLineGraphic.infoTemplate = new InfoTemplate("${lineName}", "<b>客运量: </b>${flow}人");
